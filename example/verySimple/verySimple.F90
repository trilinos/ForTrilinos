program main
 ...
 ! * b) methods implemented only in BlockMap can be invoked on the Map */
  ...
  mapID = Epetra_Map_Create(numGlobalElements, indexBase=0, commID);
 ...
 !/* Do some vector operations */
  call Epetra_MultiVector_PutScalar(mbID, 2.0);
  call Epetra_MultiVector_Update_WithA(mxID, 2.0, mbID, 0.0);!/* x = 2*b */

  call Epetra_MultiVector_Norm2(mbID, bnorm);
  call Epetra_MultiVector_Norm2(mxID, xnorm);
